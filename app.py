...
EXCEL_COL_PRIMARY_DB = 5    # F - Primary ID
EXCEL_COL_SECONDARY_DB = 6  # G - Secondary ID
EXCEL_COL_SHOPTEST = 7      # H - Shop Test Date
...
if uploaded_sheet and uploaded_db:
    if st.button("🚀 Update Data", use_container_width=True):
        try:
            ...
            # Normalise a lookup key: strip, lowercase, remove trailing ".0"
            def clean_key(v: str) -> str:
                return str(v).strip().lower().removesuffix(".0")
            ...
            for i in range(rows_checked):
                ...
                if val_handover is not None:
                    try:
                        csv_date = pd.to_datetime(val_handover, dayfirst=False, errors="raise")
